...
export const MacbookScroll = ({
  ...
  return (
    <div
      ref={ref}
      className={cn(
        "min-h-[200vh] flex flex-col items-center py-0 md:py-80 justify-start flex-shrink-0 [perspective:800px]",
        "scale-[0.5] sm:scale-[0.65] md:scale-75 lg:scale-100"
      )}
    >
      <motion.h2
        style={{
...
export const Lid = ({
  ...
  const downloadFile = (fileUrl: string, fileName: string, setIsDownloading: (val: boolean) => void) => {
    toast({
      ...
      description: `Veuillez patienter pendant le téléchargement de ${fileName}.`,
      variant: "default",
    });
  ...
  return (
    <div className="relative [perspective:800px]">
      ...
      <motion.div
        ...
        className="h-96 w-[32rem] border absolute inset-0 bg-background text-foreground rounded-2xl p-2"
      >
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button
                    variant="outline"
                    onClick={() =>
                      downloadFile("/BILYJ-Nolhan-CV.pdf", "BILYJ-Nolhan-CV.pdf", setIsDownloadingCV)
                    }
